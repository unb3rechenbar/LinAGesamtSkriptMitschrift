...
\begin{pmatrix*}L_1\\ L_2\end{pmatrix*}\stackrel{\left(\begin{smallmatrix}Z_1\\ Z_2\end{smallmatrix}\right)\leftarrow\left(\begin{smallmatrix}aZ_1+bZ_2\\ cZ_1+dZ_2\end{smallmatrix}\right)}{\sim}\begin{pmatrix*}aL_1+bL_2\\ cL_1+dL_2\end{pmatrix*}\stackrel{\left(\begin{smallmatrix}Z_1\\ Z_2\end{smallmatrix}\right)\leftarrow\left(\begin{smallmatrix}dZ_1-bZ_2\\ -cZ_1+aZ_2\end{smallmatrix}\right)}{\sim}\begin{pmatrix*}L_1\\ L_2\end{pmatrix*}\\
};
\end{tikzpicture}\\
...
\end{tikzpicture}:=T.
\end{align*}
\oe sei $n\ge 2$. Es gilt $d_n(S)=\det(S)=p_1\hdots p_m=\det(T)=d_n(T)$ und $d_{n-1}(S)=1=d_{n-1}(T)$, denn $d_{n_1}(T)$ teilt $p_1\hdots p_{i-1}p_{i+1}\hdots p_m$ für alle $i\in\{1,...,m\}$. Wäre nämlich $d_{n-1}(T)\neq 1$, so gäbe es ein Primpolynom $g\in K[X]$ mit $g\mid d_{n-1}(T)$ und daher gilt $g\mid p_i$ für ein $i\in\{2,...,m\}$, sowie $g\mid p_j$ für ein $j\in\{1,...,m\}\setminus\{i\}$. Es folgt $g\mid \gcd(p_i,p_j)=1$, was ein Widerspruch ist. Daher gilt $d(S)=(1,...,1,p_1\hdots p_m)=d(T)$ und somit $S\sim T$. 
\end{cproof}
